...
CLASSIFIER_TIMEOUT_SECONDS = int(os.getenv("CHATBOT_CLASSIFIER_TIMEOUT_SECONDS", "90"))
...
- "Please count the number of documents relating to Bills Committee Meeting in May 2025"
    ...
    -> conditions includes at least:
         ...
         2) {{"field":"folderPath","operator":"contains","value":"2025.05","group":0,"join":""}}
...
def _extract_statistics_plan_with_llm(question: str) -> dict[str, Any] | None:
    payload = _call_classifier_llm(_build_statistics_mode_prompt(question), stage="StatisticsMode")
...
def _execute_statistics_metadata_search(question: str) -> dict[str, Any]:
    ...
    if date_filter_field != "none" and start_date:
        es_start = convert_to_es_datetime_start(start_date)
        if es_start:
            date_filter["gte"] = es_start
    if date_filter_field != "none" and end_date:
        es_end = convert_to_es_datetime_end(end_date)
        if es_end:
            date_filter["lte"] = es_end
    filter_clauses = _build_filter_clauses(req, {})
    if date_filter and date_filter_field != "none":
        filter_clauses.append({"range": {date_filter_field: date_filter}})

    must_clause: list[dict[str, Any]] = []
    statistics_text_fields = [
        "folder.parent.name",
        "folder.parent.name.keyword",
        "folder_path",
        "folder_name",
        "title",
        "title.keyword",
    ...
    condition_bool = _build_statistics_condition_bool(schema_conditions)
    if condition_bool is not None:
        must_clause.append({"bool": condition_bool})
    elif query:
        must_clause.append(
            {
                "multi_match": {
                    "query": query,
                    "fields": statistics_text_fields,
                    "operator": "and",
                    "lenient": True,
                }
            }
    ...
    if must_clause or filter_clauses:
        query_body = {"bool": {}}
        if must_clause:
    ...
    body = {
        ...
        "query": query_body,
        "aggs": {
            "subfolder_count": {"cardinality": {"field": "folder_path.keyword"}},
            "top_folder_path": {"terms": {"field": "folder_path.keyword", "size": 1}},
        },
    }

    _log_workflow_step(
        3,
        "Statistics OpenSearch request",
        {
    ...
    search_kwargs = _build_search_request_kwargs(SEARCH_PREFERENCE)
    response = es.search(index=SEARCH_INDEX_NAME, body=body, **search_kwargs)
    hits_obj = (response.get("hits") or {})
    total_obj = hits_obj.get("total", 0)
    total_docs = total_obj.get("value", 0) if isinstance(total_obj, dict) else int(total_obj or 0)
    aggs = response.get("aggregations", {}) or {}
    subfolder_count = int(((aggs.get("subfolder_count") or {}).get("value") or 0))
    top_folder_buckets = ((aggs.get("top_folder_path") or {}).get("buckets") or [])
    top_folder_path = str(top_folder_buckets[0].get("key") or "").strip() if top_folder_buckets else ""

    if total_docs <= 0:
        answer = "No matching documents were found for statistics generation."
    elif top_folder_path:
        answer = (
            f"A total of {total_docs} documents are contained in {subfolder_count} "
            f"sub-folders of \"{top_folder_path}\"."
        )
    else:
    ...
    _log_workflow_step(
        4,
        "Statistics OpenSearch response",
        {
...
def _run_statistics_generation(question: str, chat_id: Optional[str]) -> dict[str, Any]:
    resolved_chat_id = chat_id or str(uuid4())
    stats = _execute_statistics_metadata_search(question)
...
def _call_classifier_llm(prompt: str, stage: str = "classifier") -> dict[str, Any] | None:
    ...
    try:
        ...
        logger.warning(
            "Step 2: %s classifier request timed out after %ss; falling back.",
            stage,
...
def _save_chat_log(chat_id: str, payload: dict[str, Any], assistant_response: str, token_usage: Optional[dict[str, Any]] = None) -> None:
    ...
    try:
        ...
        # Logging failures must not fail the chat request path.
        logger.warning("Failed to save chat log to OpenSearch: %s", exc)
...
async def agent_chat(req: ChatRequest):
    ...
    if req.statistics_generation:
        statistics_result = _run_statistics_generation(req.question, req.chat_id)
        if req.stream:
