...
_CHATBOT_ENV_PATH = os.path.abspath(os.path.join(_API_DIR, "..", ".env"))
if os.path.exists(_CHATBOT_ENV_PATH):
    load_dotenv(_CHATBOT_ENV_PATH)
else:
    load_dotenv()
...
_raw_opensearch_host = os.getenv("CHATBOT_OPENSEARCH_HOST") or os.getenv("OPENSEARCH_HOST") or "localhost"
_parsed_opensearch_host = urlparse(_raw_opensearch_host) if "://" in _raw_opensearch_host else None
OPENSEARCH_HOST = (
    _parsed_opensearch_host.hostname
    if _parsed_opensearch_host and _parsed_opensearch_host.hostname
    else _raw_opensearch_host
)
OPENSEARCH_PORT = int(
    os.getenv("CHATBOT_OPENSEARCH_PORT")
    or os.getenv("OPENSEARCH_PORT")
    or (_parsed_opensearch_host.port if _parsed_opensearch_host and _parsed_opensearch_host.port else 9200)
)
OPENSEARCH_USERNAME = os.getenv("CHATBOT_OPENSEARCH_USERNAME") or os.getenv("OPENSEARCH_USERNAME") or "admin"
OPENSEARCH_PASSWORD = os.getenv("CHATBOT_OPENSEARCH_PASSWORD") or os.getenv("OPENSEARCH_PASSWORD") or "ASLgemini916"
OPENSEARCH_USE_SSL = (
    os.getenv("CHATBOT_OPENSEARCH_USE_SSL")
    or os.getenv("OPENSEARCH_USE_SSL")
    or ("true" if (_parsed_opensearch_host and _parsed_opensearch_host.scheme == "https") else "false")
).lower() == "true"
OPENSEARCH_VERIFY_CERTS = (
    os.getenv("CHATBOT_OPENSEARCH_VERIFY_CERTS")
    or os.getenv("OPENSEARCH_VERIFY_CERTS")
    or "false"
).lower() == "true"
...
LLM_ENABLE_THINKING = os.getenv("LLM_ENABLE_THINKING", "false").strip().lower() == "true"
LLM_EXTRA_BODY_JSON = os.getenv("LLM_EXTRA_BODY_JSON", "").strip()
LLM_ALLOW_LOCAL_FALLBACK = os.getenv("LLM_ALLOW_LOCAL_FALLBACK", "false").strip().lower() == "true"
...
LLM_NON_STREAM_READ_TIMEOUT_SECONDS = float(os.getenv("LLM_NON_STREAM_READ_TIMEOUT_SECONDS", "180"))
LLM_STREAM_READ_TIMEOUT_SECONDS = float(os.getenv("LLM_STREAM_READ_TIMEOUT_SECONDS", "600"))
...
LLM_THINK_TAG_TOKEN_IDS = {
    int(value.strip())
    for value in os.getenv("LLM_THINK_TAG_TOKEN_IDS", "248069").split(",")
...
CHATBOT_REQUEST_QUOTA_ENABLED = os.getenv("CHATBOT_REQUEST_QUOTA_ENABLED", "false").lower() == "true"
CHATBOT_REQUEST_QUOTA_LIMIT = int(os.getenv("CHATBOT_REQUEST_QUOTA_LIMIT", "60"))
CHATBOT_REQUEST_QUOTA_WINDOW_SECONDS = int(os.getenv("CHATBOT_REQUEST_QUOTA_WINDOW_SECONDS", "60"))
...
_chatbot_request_quota_bucket: dict[str, list[float]] = {}
_chatbot_request_quota_lock = threading.Lock()
_llm_http_session = requests.Session()
_llm_http_adapter = HTTPAdapter(pool_connections=50, pool_maxsize=50, max_retries=0)
_llm_http_session.mount("http://", _llm_http_adapter)
_llm_http_session.mount("https://", _llm_http_adapter)


def _decode_basic_auth_username(authorization_header: str) -> str:
...
def _resolve_request_identity(request: Request) -> str:
    username = _decode_basic_auth_username(request.headers.get("authorization", ""))
    if username:
        return f"user:{username}"

    forwarded_user = str(request.headers.get("x-forwarded-user", "")).strip()
    if forwarded_user:
...
def _consume_quota_token(bucket: dict[str, list[float]], lock: threading.Lock, key: str, limit: int, window_seconds: int) -> tuple[bool, int]:
    now = time.time()
    cutoff = now - float(window_seconds)
    with lock:
        timestamps = [ts for ts in bucket.get(key, []) if ts >= cutoff]
        if len(timestamps) >= limit:
            bucket[key] = timestamps
            return False, 0
        timestamps.append(now)
        bucket[key] = timestamps
        return True, max(0, limit - len(timestamps))


def _enforce_chatbot_request_quota(request: Request, endpoint_name: str) -> None:
    if not CHATBOT_REQUEST_QUOTA_ENABLED:
        return
    if CHATBOT_REQUEST_QUOTA_LIMIT <= 0 or CHATBOT_REQUEST_QUOTA_WINDOW_SECONDS <= 0:
        return

    identity = _resolve_request_identity(request)
    allowed, remaining = _consume_quota_token(
        _chatbot_request_quota_bucket,
        _chatbot_request_quota_lock,
        key=identity,
        limit=CHATBOT_REQUEST_QUOTA_LIMIT,
        window_seconds=CHATBOT_REQUEST_QUOTA_WINDOW_SECONDS,
    )
    if allowed:
        _log_verbose(
            "Chatbot quota token consumed",
            {
                "identity": identity,
    ...
    raise HTTPException(
        status_code=429,
        detail=(
            f"Too many chatbot requests for {identity}. "
            f"Limit is {CHATBOT_REQUEST_QUOTA_LIMIT} requests per {CHATBOT_REQUEST_QUOTA_WINDOW_SECONDS} seconds."
        ),
    )
...
def _build_chat_user_prompt(req: ChatRequest) -> str:
    if req.press_releases:
        ...
        return (
            f"You are a helpful assistant. Answer all questions based on the provided document content. Question: {req.question}\n"
            f"Here is the document content:\n\n{combined}"
        )

    return (
        "You are a helpful assistant. Please answer based on the provided context. "
        "Do not answer questions that are unrelated to the context topic. "
        "Respond in the same language as the user's question. "
        f"My question is: {req.question}"
    )
...
def _process_chat_request(req: ChatRequest, request_id: Optional[str] = None):
    ...
    user_prompt = _build_chat_user_prompt(req)
    if req.stream:
        generator = _generate_completion_stream(req.chat_id, user_prompt, request_id=resolved_request_id)
        return StreamingResponse(generator, media_type="text/event-stream")
    return _generate_completion(req.chat_id, user_prompt, request_id=resolved_request_id)
...
def _extract_upstream_context_limits(error_text: str) -> tuple[Optional[int], Optional[int]]:
    if not error_text:
        return None, None
    try:
    ...
    if isinstance(prompt_tokens, int) and isinstance(context_limit, int):
        return prompt_tokens, context_limit
    return None, None


def _is_upstream_context_size_error(status_code: int, error_text: str) -> bool:
    if status_code != 400:
        return False
...
def _build_context_size_error_detail(error_text: str) -> dict[str, Any]:
    current_tokens, max_tokens = _extract_upstream_context_limits(error_text)
    detail: dict[str, Any] = {
    ...
    if isinstance(current_tokens, int):
        detail["current_tokens"] = current_tokens
    return detail
...
def _resolved_llm_provider() -> str:
    if LLM_PROVIDER in {"ollama", "vllm", "openai", "completion", "llamacpp", "llama_cpp"}:
        if LLM_PROVIDER in {"llamacpp", "llama_cpp"}:
            return "completion"
        return LLM_PROVIDER
    if "/v1/chat/completions" in (LLM_API or ""):
        return "vllm"
    if "/completion" in (LLM_API or ""):
        return "completion"
    return "ollama"
...
def _llm_api_candidates() -> list[str]:
    provider = _resolved_llm_provider()
    primary = (LLM_API or "").strip()
    candidates = [primary]

    parsed = urlparse(primary) if primary else None
    if provider == "ollama":
        # Try equivalent endpoint styles on the same host first.
        if parsed and parsed.scheme and parsed.netloc:
            if parsed.path.endswith("/v1/chat/completions"):
                candidates.append(f"{parsed.scheme}://{parsed.netloc}/api/chat")
            elif parsed.path.endswith("/api/chat"):
                candidates.append(f"{parsed.scheme}://{parsed.netloc}/v1/chat/completions")

        # Only if explicitly enabled, try local Ollama defaults after configured host.
        host = (parsed.hostname or "").lower() if parsed else ""
        if LLM_ALLOW_LOCAL_FALLBACK and host not in {"localhost", "127.0.0.1", "::1"}:
            candidates.extend(
                [
...
def _resolve_llm_http_timeout(stream: bool, timeout: Optional[float]) -> tuple[float, float]:
    read_timeout = timeout
    if read_timeout is None or read_timeout <= 0:
        read_timeout = LLM_STREAM_READ_TIMEOUT_SECONDS if stream else LLM_NON_STREAM_READ_TIMEOUT_SECONDS
    connect_timeout = LLM_CONNECT_TIMEOUT_SECONDS if LLM_CONNECT_TIMEOUT_SECONDS > 0 else 10.0
    return (connect_timeout, float(read_timeout))


def _post_llm(payload: dict[str, Any], *, stream: bool = False, timeout: Optional[float] = None, api_override: Optional[str] = None) -> tuple[requests.Response, str]:
    targets = [api_override] if api_override else _llm_api_candidates()
    errors: list[str] = []
    provider = _resolved_llm_provider()
    timeout_tuple = _resolve_llm_http_timeout(stream, timeout)
    for api in _dedupe_preserve_order(targets):
        try:
            response = _llm_http_session.post(
                api,
                json=payload,
            ...
            # For completion servers under concurrent load, allow one longer retry.
            if provider == "completion":
                retry_timeout = _resolve_llm_http_timeout(stream, timeout_tuple[1] * 2)
                try:
                    response = _llm_http_session.post(
                        api,
                        json=payload,
                    ...
                    errors.append(f"{api} retry -> {retry_exc}")
    ...
    raise HTTPException(
        ...
        detail=(
            "LLM request failed. "
            f"provider={provider} configured={LLM_API} tried={targets} timeout={timeout_tuple} errors={errors}"
        ),
    )
...
def _extract_stream_chunk_and_done(data: dict[str, Any], provider: str) -> tuple[str, bool]:
    # Detect OpenAI/vLLM response shape by payload regardless of configured provider.
    # Some llama servers expose /v1/chat/completions SSE format even when provider is set differently.
    has_choices_shape = isinstance(data.get("choices"), list)
    if provider in {"vllm", "openai"} or has_choices_shape:
    ...
    if provider == "completion":
        chunk = str(data.get("content") or data.get("token") or data.get("completion") or data.get("response") or "")
        done = bool(
            data.get("stop") is True
            or data.get("done") is True
            or data.get("completed") is True
            or data.get("finish_reason") is not None
...
def _extract_stream_token_ids(data: dict[str, Any]) -> set[int]:
    ...
    for token in raw_tokens:
        try:
            token_ids.add(int(token))
        except (TypeError, ValueError):
            continue
    return token_ids


def _strip_think_stream_chunk(chunk: str, data: dict[str, Any], state: dict[str, bool]) -> str:
    if not chunk:
        return ""

    text = chunk
    token_ids = _extract_stream_token_ids(data)
    if LLM_THINK_TAG_TOKEN_IDS and token_ids.intersection(LLM_THINK_TAG_TOKEN_IDS):
        # Token IDs are hints for think-tag boundaries; do not discard full chunks blindly.
        # Some servers may include boundary token IDs together with normal answer text.
        if "</think>" in text:
            state["in_think"] = False
        elif "<think>" in text:
            state["in_think"] = True
        # Marker token without explicit tag text can appear inside normal content.
        # Never change filtering mode based on token IDs alone.
...
def _build_classifier_payload(prompt: str) -> dict[str, Any]:
    provider = _resolved_llm_provider()
    llama_style_endpoint = "/v1/chat/completions" in (LLM_API or "") or "/completion" in (LLM_API or "")
    if provider in {"vllm", "openai"}:
        ...
        if not LLM_ENABLE_THINKING and llama_style_endpoint:
            payload.setdefault("chat_template_kwargs", {"enable_thinking": False})
        return payload

    if provider == "completion":
        completion_prompt = (
            f"System: {CLASSIFIER_SYSTEM_PROMPT}\n\n"
            f"User: {prompt}\n\n"
            "Assistant:"
        )
        payload = {
...
def _build_reduced_payload_for_retry(payload: dict[str, Any], provider: str) -> dict[str, Any]:
    ...
    if provider in {"vllm", "openai"}:
        ...
        # Keep compatibility if an old payload still contains `parameters`.
...
def _build_vllm_extra_body() -> dict[str, Any] | None:
    if not LLM_EXTRA_BODY_JSON:
        return None

    try:
        parsed = json.loads(LLM_EXTRA_BODY_JSON)
    except json.JSONDecodeError as exc:
        logger.warning("Failed to parse LLM_EXTRA_BODY_JSON: %s", exc)
        return None

    if not isinstance(parsed, dict):
        logger.warning("LLM_EXTRA_BODY_JSON must be a JSON object")
        return None
...
def _stream_text_response(
    ...
    def _generator():
        ...
        for idx in range(0, len(answer), 50):
            yield f"data: {json.dumps({'chunk': answer[idx:idx+50], 'chat_id': chat_id, 'request_id': resolved_request_id, 'complete': False})}\n\n"
        final_payload = {"complete": True, "chat_id": chat_id, "request_id": resolved_request_id, "intent": intent.value}
        if extra_meta:
...
def _wrap_stream_with_metadata(
    generator,
    intent: IntentType,
    extra_meta: Optional[dict[str, Any]] = None,
    request_id: Optional[str] = None,
...
async def _build_agent_stream(
    ...
    if intent == IntentType.KEYWORD_SEARCH:
        ...
        return _wrap_stream_with_metadata(
            _stream_text_response(answer, resolved_chat_id, intent, request_id=resolved_request_id),
            intent,
    ...
    if intent == IntentType.STATS_COUNT:
        ...
        return _stream_text_response(answer, resolved_chat_id, intent, classification_meta, request_id=resolved_request_id)
    ...
    if intent == IntentType.GENERAL_RAG_QA:
        ...
        rag_prompt = (
           ...
           Instructions:
            1. answer language: English!!!!If user explicitly says "Reply in X", obey that.
            2. Scan all documents to identify which contain information relevant to the question.
            3. From relevant documents, extract the specific facts, numbers, or details that answer the question.
            4. State your answer directly, including all specific details (numbers, dates, amounts) from the documents.
            5. Cite which document and page the information came from.
            6. **Select the most similar case**: If multiple cases are provided, compare them and identify which one is most factually and legally similar to the user's question. Justify your selection by explaining the similarities.
            7. **Focus on the selected case**: After identifying the most similar case, center your answer on it. Other cases may be mentioned briefly for comparison only, but the primary answer must be based on the selected case.
            8. **Quote the exact wording**: When you find a sentence or phrase that directly answers the user's question, quote it verbatim using quotation marks. 
            9. **Before citing a page number, you MUST look for the nearest "------[Page X:]" marker that appears BEFORE the quoted text in the provided context. Use that number.**
            10. **Do not mention unrelated documents or explain why they are not relevant, as this distracts from the answer.**
            
            
            Important:
            - answer language must be based on the LANGUAGE RULES above
        ...
        generator = _generate_completion_stream(
            resolved_chat_id,
            rag_prompt,
    ...
    if intent == IntentType.FREE_OPEN_CHAT:
        ...
        return _wrap_stream_with_metadata(generator, intent, classification_meta, request_id=resolved_request_id)
    ...
    if intent == IntentType.SINGLE_DOC_SUMMARY:
        ...
        title = source.get("title", "Untitled")
        summary_prompt = f"Please provide a brief summary (2-3 sentences) of the following document:\nTitle: {title}\n\nContent: {content[:3000]}"
        generator = _generate_completion_stream(resolved_chat_id, summary_prompt, request_id=resolved_request_id)
        return _wrap_stream_with_metadata(
            generator,
            intent,
            {"doc_id": doc_id, "title": title, **classification_meta},
            request_id=resolved_request_id,
        )
    ...
    if intent == IntentType.MIXED_SEARCH_SUMMARY:
        ...
        return _wrap_stream_with_metadata(
            generator,
            intent,
            {"sources": [_summarize_result_for_log(item) for item in results], **classification_meta},
            request_id=resolved_request_id,
        )
    ...
    return _stream_text_response(
        ...
        intent,
        classification_meta,
        request_id=result.get("request_id") or resolved_request_id,
    )
...
def _merge_dms_forward_headers(forwarded_headers: Optional[dict[str, str]]) -> dict[str, str]:
    ...
    if not isinstance(forwarded_headers, dict):
        return headers
    for header_name in ("Authorization", "Cookie", "X-Auth-Token", "X-CSRF-Token", "X-Client-Session-Id"):
...
def _merge_classifier_parameters(base_parameters: dict[str, Any], classifier_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    # Ignore category to prevent accidental over-filtering from classifier output.
    merged["category"] = []
    merged.pop("categories", None)
    # Ignore tags to prevent accidental over-filtering (e.g. "similar case").
    merged["tags"] = []
...
def _call_classifier_llm(
    ...
    try:
        response, used_api = _post_llm(payload, stream=False, timeout=resolved_timeout)
    except requests.exceptions.Timeout:
        logger.warning("Step 2: %s classifier timed out after %ss", stage, resolved_timeout)
        return None
    except HTTPException as exc:
        logger.warning("Step 2: %s classifier request failed: %s", stage, exc.detail)
        return None

    logger.info("Step 2: %s classifier endpoint used: %s", stage, used_api)
    ...
    cleaned_raw = remove_deepthink(str(raw_content or "")).strip()
    content = re.sub(r'```json\s*|\s*```', '', cleaned_raw)
...
- Fill start_date, end_date, owners, folder_names, folder_paths, and metadata_filters when the user clearly specifies filters.
- Do not use category or tags as filters.
- Always return "category": [] and "tags": [].
...
- If the only filter is a date, owner, or folder (no keyword terms), use hybrid_search, not keyword_search.
...
async def _classify_intent_with_llm(question: str, base_parameters: dict[str, Any]) -> IntentResult | None:
    ...
    if search_strategy == SearchStrategy.HYBRID_SEARCH:
        # In hybrid mode, do not constrain retrieval with structured filters.
        merged_parameters["start_date"] = ""
        merged_parameters["end_date"] = ""
        merged_parameters["owners"] = []
        merged_parameters["folder_names"] = []
        merged_parameters["folder_paths"] = []
        merged_parameters["metadata_filters"] = {}
...
async def orchestrate_agent_chat(
    ...
    try:
        if intent == IntentType.KEYWORD_SEARCH:
            ...
            agent_result = _generate_completion(
                resolved_chat_id,
                question,
                system_prompt=OPEN_CHAT_SYSTEM_PROMPT,
                request_id=resolved_request_id,
...
def _build_llm_payload(messages: list[dict[str, str]], stream: bool) -> dict[str, Any]:
    ...
    if provider == "completion":
        ...
        if not LLM_ENABLE_THINKING and llama_style_endpoint:
            payload["chat_template_kwargs"] = {"enable_thinking": False}
        _log_verbose("Built LLM payload", {"provider": provider, "stream": stream, "n_predict": LLM_MAX_TOKENS})
        return payload
    ...
    print("max_tokens:", LLM_MAX_TOKENS)
    print("temperature:", LLM_TEMPERATURE)
    print("top_p:", LLM_TOP_P)
    _log_verbose("Built LLM payload", _summarize_llm_payload_for_log(payload))
    return payload
...
def _generate_completion(
    ...
    response, used_api = _post_llm(payload, stream=False)
    _log_verbose("Non-stream LLM endpoint used", {"chat_id": resolved_chat_id, "llm_api": used_api})
    if not response.ok:
        provider = _resolved_llm_provider()
        raw_error_text = response.text or ""
        error_preview = _preview_text(raw_error_text, 500)
        if response.status_code == 400 and provider in {"vllm", "openai"}:
            ...
            retry_response, retry_api = _post_llm(retry_payload, stream=False, api_override=used_api)
            if retry_response.ok:
                response = retry_response
                _log_verbose("Non-stream LLM retry endpoint used", {"chat_id": resolved_chat_id, "llm_api": retry_api})
            else:
                retry_raw_error_text = retry_response.text or ""
                retry_error_preview = _preview_text(retry_response.text, 500)
                if _is_upstream_context_size_error(retry_response.status_code, retry_raw_error_text):
                    raise HTTPException(status_code=400, detail=_build_context_size_error_detail(retry_raw_error_text))
                raise HTTPException(
                    status_code=502,
                    detail=f"LLM API error: {retry_response.status_code} | {retry_error_preview}",
                )
        else:
            if _is_upstream_context_size_error(response.status_code, raw_error_text):
                raise HTTPException(status_code=400, detail=_build_context_size_error_detail(raw_error_text))
...
def _generate_completion_stream(
    chat_id: Optional[str],
    user_prompt: str,
    system_prompt: Optional[str] = None,
    request_id: Optional[str] = None,
    ...
    def _event_generator():
        ...
        try:
            ...
            response, used_api = _post_llm(payload, stream=True)
            _log_verbose("Stream LLM endpoint used", {"chat_id": resolved_chat_id, "llm_api": used_api})
            if response.status_code != 200:
                ...
                if response.status_code == 400 and provider in {"vllm", "openai"}:
                    ...
                    response, _ = _post_llm(retry_payload, stream=True, api_override=used_api)
                    if response.status_code != 200:
                        retry_raw_error_text = response.text or ""
                        retry_error_preview = _preview_text(retry_raw_error_text, 500)
                        if _is_upstream_context_size_error(response.status_code, retry_raw_error_text):
                            context_error = _build_context_size_error_detail(retry_raw_error_text)
                            context_error.update(
                                {
                                    "chat_id": resolved_chat_id,
                                    "request_id": resolved_request_id,
                                    "complete": True,
                            ...
                            yield f"data: {json.dumps(context_error)}\n\n"
                            return
                        yield f"data: {json.dumps({'error': f'LLM API error: {response.status_code} | {retry_error_preview}', 'chat_id': resolved_chat_id, 'request_id': resolved_request_id, 'complete': True})}\n\n"
                        return
                else:
                    if _is_upstream_context_size_error(response.status_code, raw_error_text):
                        context_error = _build_context_size_error_detail(raw_error_text)
                        context_error.update(
                            {
                                "chat_id": resolved_chat_id,
                                "request_id": resolved_request_id,
                        ...
                        yield f"data: {json.dumps(context_error)}\n\n"
                        return
                    yield f"data: {json.dumps({'error': f'LLM API error: {response.status_code} | {error_preview}', 'chat_id': resolved_chat_id, 'request_id': resolved_request_id, 'complete': True})}\n\n"
                    return
            for line in response.iter_lines():
                ...
                if chunk:
                    raw_chunk_count += 1
                    raw_assistant_response += chunk
                if provider == "completion" and not LLM_ENABLE_THINKING:
                    chunk = _strip_think_stream_chunk(chunk, data, strip_state)
                if done:
                    final_event = data
                    _log_workflow_step(
                        5,
                ...
                yield f"data: {json.dumps({'chunk': chunk, 'chat_id': resolved_chat_id, 'request_id': resolved_request_id, 'complete': False})}\n\n"
            cleaned = assistant_response.strip()
            logger.info(
                "Step 5: Stream completion stats | chat_id=%s chunk_count=%s raw_chunk_count=%s response_chars=%s raw_response_chars=%s",
                resolved_chat_id,
                chunk_count,
                raw_chunk_count,
                len(cleaned),
                len(raw_assistant_response.strip()),
            )
            if not cleaned:
                if raw_chunk_count > 0:
                    # Stream returned content, but it was suppressed (for example think-tag stripping).
                    # Derive final text from raw stream without forcing a second blocking non-stream request.
                    cleaned = remove_deepthink(raw_assistant_response).strip()

                if cleaned:
                    yield f"data: {json.dumps({'chunk': cleaned, 'chat_id': resolved_chat_id, 'request_id': resolved_request_id, 'complete': False})}\n\n"
            ...
            if not cleaned:
                ...
                fallback_response, _ = _post_llm(fallback_payload, stream=False)
            ...
            yield f"data: {json.dumps({'complete': True, 'chat_id': resolved_chat_id, 'request_id': resolved_request_id, 'token_usage': token_usage})}\n\n"
        except HTTPException as exc:
            yield f"data: {json.dumps({'error': str(exc.detail), 'chat_id': resolved_chat_id, 'request_id': resolved_request_id, 'complete': True})}\n\n"
        except Exception as exc:
            yield f"data: {json.dumps({'error': str(exc), 'chat_id': resolved_chat_id, 'request_id': resolved_request_id, 'complete': True})}\n\n"
...
def chat(req: ChatRequest, request: Request):
    _enforce_chatbot_request_quota(request, endpoint_name="/api/chatbot/chat")
    return _process_chat_request(req, request_id=_new_request_id())
...
async def agent_chat(req: ChatRequest, request: Request):
    ...
    if not req.question.strip():
        raise HTTPException(status_code=400, detail="Missing question")
    _enforce_chatbot_request_quota(request, endpoint_name="/api/chatbot/agent-chat")
    _log_workflow_step(
        1,
    ...
    if req.statistics_generation:
        ...
        if req.stream:
            generator = _stream_text_response(
                ...
                request_id=statistics_result.get("request_id"),
            )
            return StreamingResponse(generator, media_type="text/event-stream")
...
def summarize_multiple(req: SummarizeRequest, request: Request):
    _enforce_chatbot_request_quota(request, endpoint_name="/api/chatbot/summarize-multiple")
    request_id = _new_request_id()
    ...
    user_prompt = (
        ...
        "If user question is provided, reply in the SAME language as the user question. "
        "If no user question is provided, determine the dominant language from the provided document context and reply in that language; "
        "if the context language is mixed or unclear, default to English. "
        f"Please answer the user question based on these {len(req.press_releases)} documents, and include document title references in your answer."
        f"\n\nUser question:\n{user_question if user_question else '[not provided]'}"
        f"\n\nDocument context:\n{combined}"
    )
    if req.stream:
        generator = _generate_completion_stream(req.chat_id, user_prompt, request_id=request_id)
        return StreamingResponse(generator, media_type="text/event-stream")
    result = _generate_completion(req.chat_id, user_prompt, request_id=request_id)
    return {"chat_id": result["chat_id"], "request_id": result.get("request_id"), "summary": result["answer"]}
