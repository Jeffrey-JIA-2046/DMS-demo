...
[ "_main", "__emscripten_thread_init", "__emscripten_thread_exit", "__emscripten_thread_crashed", "__emscripten_thread_mailbox_await", "__emscripten_tls_init", "_pthread_self", "checkMailbox", "__embind_initialize_bindings", "establishStackSpace", "invokeEntryPoint", "PThread", "_module_set", "___indirect_function_table", "_fflush", "___asyncjs__em_libusb_wait_async", "_usbi_em_promise_catch", "_usbi_em_copy_from_dataview", "_usbi_em_device_safe_open_close", "_usbi_em_cancel_transfer_promise", "_usbi_em_wrap_transfer_promise", "___emscripten_embedded_file_data", "___start_em_js", "___stop_em_js", "onRuntimeInitialized" ].forEach(prop => {
...
*/ {
 ...
 const libFunctionsAsync = {
  sane_get_state: false,
  sane_init: false,
  sane_exit: true,
  sane_get_devices: true,
  sane_open: true,
  sane_close: true,
  sane_get_option_descriptor: false,
  sane_control_option_get_value: true,
  sane_control_option_set_value: true,
  sane_control_option_set_auto: true,
  sane_get_parameters: true,
  sane_start: true,
  sane_read: true,
  sane_cancel: true,
  sane_strstatus: false
 };
 Module.sane = {
  debugSANE: false,
  debugUSB: false,
...
function usbi_em_cancel_transfer_promise(transfer) {
...
function usbi_em_wrap_transfer_promise(transfer, handle) {
 let cancel;
 const promise = Emval.toValue(handle);
 const promiseWrapped = Promise.race([ promise, new Promise((resolve, reject) => cancel = reject) ]).finally(() => {
  delete Module.libusbTransferPromises[transfer];
 });
...
var PThread = {
 ...
 allocateUnusedWorker() {
  var worker;
  var pthreadMainJs = locateFile("libsane.worker.js");
...
function ___syscall_recvfrom(fd, buf, len, flags, addr, addrlen) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(16, 1, fd, buf, len, flags, addr, addrlen);
 try {
  var sock = getSocketFromFD(fd);
...
function ___syscall_sendto(fd, message, length, flags, addr, addr_len) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(18, 1, fd, message, length, flags, addr, addr_len);
 try {
  var sock = getSocketFromFD(fd);
...
function ___syscall_socket(domain, type, protocol) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(19, 1, domain, type, protocol);
 try {
...
function __mmap_js(len, prot, flags, fd, offset_low, offset_high, allocated, addr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(23, 1, len, prot, flags, fd, offset_low, offset_high, allocated, addr);
...
function __munmap_js(addr, len, prot, flags, fd, offset_low, offset_high) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(24, 1, addr, len, prot, flags, fd, offset_low, offset_high);
 var offset = convertI32PairToI53Checked(offset_low, offset_high);
 try {
  if (isNaN(offset)) return 61;
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function __setitimer_js(which, timeout_ms) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(25, 1, which, timeout_ms);
 if (timers[which]) {
...
var _environ_get = function(__environ, environ_buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(26, 1, __environ, environ_buf);
 var bufSize = 0;
...
var _environ_sizes_get = function(penviron_count, penviron_buf_size) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(27, 1, penviron_count, penviron_buf_size);
...
function _fd_seek(fd, offset_low, offset_high, whence, newOffset) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(30, 1, fd, offset_low, offset_high, whence, newOffset);
 var offset = convertI32PairToI53Checked(offset_low, offset_high);
 try {
  if (isNaN(offset)) return 61;
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function _getaddrinfo(node, service, hint, out) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(32, 1, node, service, hint, out);
 var addrs = [];
 var canon = null;
 var addr = 0;
 var port = 0;
...
function _gethostbyname(name) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(33, 1, name);
...
var proxiedFunctionTable = [ _proc_exit, exitOnMainThread, pthreadCreateProxied, ___syscall__newselect, ___syscall_bind, ___syscall_chmod, ___syscall_connect, ___syscall_fcntl64, ___syscall_fstat64, ___syscall_getdents64, ___syscall_ioctl, ___syscall_lstat64, ___syscall_newfstatat, ___syscall_openat, ___syscall_pipe, ___syscall_poll, ___syscall_recvfrom, ___syscall_rmdir, ___syscall_sendto, ___syscall_socket, ___syscall_stat64, ___syscall_unlinkat, __emscripten_runtime_keepalive_clear, __mmap_js, __munmap_js, __setitimer_js, _environ_get, _environ_sizes_get, _fd_close, _fd_read, _fd_seek, _fd_write, _getaddrinfo, _gethostbyname ];
...
var wasmImports = {
 ...
 /** @export */ usbi_em_cancel_transfer_promise: usbi_em_cancel_transfer_promise,
 ...
 /** @export */ usbi_em_wrap_transfer_promise: usbi_em_wrap_transfer_promise
};
...
var __emval_coro_resume = createExportWrapper("_emval_coro_resume");
...
var ___emscripten_embedded_file_data = Module["___emscripten_embedded_file_data"] = 975476;

var ___start_em_js = Module["___start_em_js"] = 1063896;

var ___stop_em_js = Module["___stop_em_js"] = 1066133;
