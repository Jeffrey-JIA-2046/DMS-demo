...
import { fetchDocument, getStoredDocumentOcr } from '../api/documents'
import { isSessionInvalidationMessage, redirectToLogin } from '../api/httpClient'
...
function resolveDocumentTextForSummary(detail = {}, fallback = '') {
  const candidates = [
    detail?.ocr_content,
    detail?.ocrContent,
    detail?.content,
...
function resolveFullOcrText(ocrPayload) {
  const pages = Array.isArray(ocrPayload?.results) ? ocrPayload.results : []
  if (!pages.length) {
    return ''
  }
  ...
  const ordered = [...pages]
    .map((page) => ({
      ...
      text: typeof page?.md_content === 'string' ? page.md_content.trim() : '',
    }))
    .filter((entry) => entry.text)
    .sort((a, b) => {
      const left = Number.isFinite(a.pageNo) ? a.pageNo : Number.MAX_SAFE_INTEGER
      const right = Number.isFinite(b.pageNo) ? b.pageNo : Number.MAX_SAFE_INTEGER
      return left - right
    })
  ...
  return ordered
    .map((entry, index) => {
      const humanPage = Number.isFinite(entry.pageNo) ? entry.pageNo + 1 : index + 1
      return `[Page ${humanPage}]\n${entry.text}`
    })
    .join('\n\n')
}
...
export default function ChatbotPanel({ selectedDocument, onDocumentSelect, onDocumentMaximize, open: openProp, onOpenChange, folders = [] }) {
  ...
  const [chatSendDisabled, setChatSendDisabled] = useState(false)

  const lockPanelAndRedirectIfSessionInvalid = (message) => {
  ...
  const runSearch = async ({ page = 1, perPage = itemsPerPage } = {}) => {
    ...
    try {
      ...
      setError(message)
      lockPanelAndRedirectIfSessionInvalid(message)
      toast && toast(message, { type: 'error' })
    } finally {
  ...
  const handleSummarizeSelected = async () => {
    ...
    try {
      const pressReleasePayload = await Promise.all(
        selectedPressReleases.map(async (item) => {
          try {
            const detail = await fetchDocument(item.id)
            let fullDocumentContent = ''
            try {
              const ocrPayload = await getStoredDocumentOcr(item.id)
              fullDocumentContent = resolveFullOcrText(ocrPayload)
            } catch {
              fullDocumentContent = ''
            }
            ...
            const date =
              ...
              || detail?.created_at
              || item.date
            const detailMetadata = stringifyMetadata(detail?.metadata)
            const itemMetadata = stringifyMetadata(item?.metadata)
            const mergedContent = [
              fullDocumentContent || resolveDocumentTextForSummary(detail, item.content),
              detailMetadata,
              itemMetadata,
            ].filter(Boolean).join('\n\n')
      ...
      const data = await summarizeDocumentWithChatbot({
        ...
        pressReleases: pressReleasePayload,
      }, (chunk) => {
  ...
  const handleQuestion = async (event) => {
    event.preventDefault()
    if (chatSendDisabled) {
      setError('Your session ended. Please login again to continue chatting.')
      redirectToLogin()
      return
    }
    if (!question.trim()) {
  ...
  const renderPanel = () => (
    <section
      className="chatbot-panel chatbot-panel--template"
      aria-disabled={chatSendDisabled}
      style={chatSendDisabled ? { pointerEvents: 'none', opacity: 0.72 } : undefined}
    >
        ...
        <div className="chatbot-template__chat-section">
          ...
          <form className="chatbot-template__chat-input" onSubmit={handleQuestion}>
            <input
              id="chatbot-question"
              type="text"
              placeholder={chatSendDisabled
                ? 'Session ended. Please login again to continue.'
                : 'Ask anything: phrase search, similar case, case summary, or document count...'}
              value={question}
              onChange={(event) => setQuestion(event.target.value)}
              disabled={chatSendDisabled}
            />
            <button type="submit" className="primary" disabled={qaLoading || chatSendDisabled}>
              {qaLoading ? 'Sending...' : 'Send'}
            </button>
          </form>

          {chatSendDisabled && <p className="feedback feedback--error">Your session was signed out. Please login again.</p>}
          ...
          <div className="chatbot-template__chat-actions">
            ...
            {requestId && <span className="chatbot-template__active-doc">Request: {requestId}</span>}
