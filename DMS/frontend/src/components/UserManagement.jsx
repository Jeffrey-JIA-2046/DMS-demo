...
import { Roles, AuthContext } from '../contexts/AuthContext'
...
export default function UserManagement() {
  ...
  const isRootActor = currentUser?.role === 'ROOT'

  const availableRoleOptions = useMemo(() => {
    if (isRootActor) {
      return ROLE_OPTIONS
    }
    return ROLE_OPTIONS.filter((option) => option.value !== 'ROOT')
  }, [isRootActor])
  ...
  async function handleUserSubmit(e) {
    ...
    try {
      ...
      if (!isRootActor && payload.role === 'ROOT') {
        toast && toast('Only root can assign the ROOT role', { type: 'error' })
        return
      }
  ...
  async function handleUserDelete(user) {
    if (!isRootActor && user?.role === 'ROOT') {
      toast && toast('Only root can delete the root user', { type: 'error' })
      return
    }
  ...
  function startEditUser(user) {
    if (!isRootActor && user?.role === 'ROOT') {
      toast && toast('Only root can edit the root user', { type: 'error' })
      return
    }
          ...
          {loading ? (
            ...
            users.map((user) => (
              <div key={user.id} className="user-management__row">
                ...
                <div className="user-management__actions">
                  <button className="ghost" onClick={() => startEditUser(user)} disabled={!isRootActor && user.role === 'ROOT'}>Edit</button>
                  <button className="ghost icon-btn" onClick={() => handleUserDelete(user)} title={`Delete user ${user.username}`} aria-label={`Delete user ${user.username}`} disabled={!isRootActor && user.role === 'ROOT'}>
                    <span aria-hidden className="icon">🗑️</span>
                  </button>
                </div>
              </div>
