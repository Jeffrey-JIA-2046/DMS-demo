...
export default function DocumentWorkspace({ currentFunction = 'Document Management', onFindRelatedTopics = null, navigationContext = null }) {
  ...
  const canManageFolderPermissions = role === Roles.ROOT || role === Roles.SYS_ADMIN || role === Roles.USER_ADMIN
  ...
  useEffect(() => {
    ...
    if (role === Roles.ROOT || role === Roles.SYS_ADMIN) {
          ...
          <div className="workspace-ocr-card__grid">
            ...
            <section className="workspace-ocr-pane">
              ...
              {activeOcrPreviewContent ? (
                ...
                <textarea
                  className="workspace-ocr-result-raw"
                  value={ocrResult ? JSON.stringify(ocrResult, null, 2) : 'No OCR result available.'}
                  readOnly
                  rows={20}
                />
              )}
  ...
  const handleUpload = async (payload, file) => {
    ...
    try {
      ...
      if (useAiFiling) {
        ...
        const detectionReason = String(aiResult?.detection?.reason || '').trim()
        const routedFolderId = aiResult?.routing?.target_folder_id
        const preOcrConfirmed = aiResult?.pre_ocr?.performed === true
        toast && toast(
          detectedType
            ? `AI filing complete (${preOcrConfirmed ? 'pre-OCR done' : 'pre-OCR unknown'}). Detected ${detectedType}${detectionReason ? `: ${detectionReason}` : ''} and routed to folder ${routedFolderId || 'mapped target'}.`
