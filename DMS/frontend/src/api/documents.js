...
const resolveExtractionApiUrl = (path) => {
  return EXTRACTION_API_BASE ? `${EXTRACTION_API_BASE}${path}` : resolveApiUrl(path)
}
...
export const runDataExtraction = async (ocrText, metadataTemplate, formType = 'form1') => {
  ...
  const response = await fetch(resolveExtractionApiUrl('/extract'), {
    method: 'POST',
