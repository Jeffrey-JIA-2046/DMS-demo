...
@Repository
public class KnowledgeContributionRepository extends BaseOpenSearchRepository<KnowledgeContribution> {

    private static final String ENTITY_TYPE = "contribution";

    @Value("${app.opensearch.knowledge-index:dms-knowledge}")
    private String knowledgeIndex;

    @Autowired
    public KnowledgeContributionRepository(OpenSearchClient openSearchClient, ObjectMapper objectMapper) {
        super(openSearchClient, objectMapper, KnowledgeContribution.class);
    }
    ...
    @Override
    public KnowledgeContribution save(KnowledgeContribution entity) throws IOException {
    ...
    public List<KnowledgeContribution> findByTopicId(String topicId) {
        try {
            ...
            throw new RuntimeException("Failed to query contributions", ex);
        }
    }
    ...
    public int countByTopicId(String topicId) {
        try {
            ...
            return (int) openSearchClient.search(request, KnowledgeContribution.class).hits().total().value();
        } catch (IOException ex) {
            throw new RuntimeException("Failed to count contributions", ex);
        }
    }
    ...
    public List<KnowledgeContribution> findByTopicIdOrderByCreatedAtAsc(String topicId) {
        try {
            ...
            List<KnowledgeContribution> results = search(request);
            results.sort(Comparator.comparing(KnowledgeContribution::getCreatedAt, Comparator.nullsLast(Comparator.naturalOrder())));
            return results;
        } catch (IOException ex) {
            throw new RuntimeException("Failed to query contributions", ex);
        }
    ...
    public List<KnowledgeContribution> findByTopicIdOrderByCreatedAtDesc(String topicId) {
        try {
            ...
            List<KnowledgeContribution> results = search(request);
            results.sort(Comparator.comparing(KnowledgeContribution::getCreatedAt, Comparator.nullsLast(Comparator.reverseOrder())));
            return results;
        } catch (IOException ex) {
            throw new RuntimeException("Failed to query contributions", ex);
        }
