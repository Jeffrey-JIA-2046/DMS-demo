...
public class ChatbotDocumentIndexService {
    ...
    private static final int HOUSEKEEPING_BATCH_SIZE = 100;
    ...
    private String primaryDocumentsIndex;
    ...
    private ChunkHousekeepingResult runChunkHousekeeping() {
        ...
        try {
            ...
            for (Hit<Map> hit : hits) {
                ...
                scannedChunks++;
                try {
                    if (!primaryDocumentExists(documentId)) {
                        DeleteRequest deleteRequest = new DeleteRequest.Builder()
                            .index(chatbotChunksIndex)
                            .id(hit.id())
                            .refresh(Refresh.WaitFor)
    ...
    private boolean primaryDocumentExists(String documentId) {
        try {
            return openSearchClient.exists(e -> e.index(primaryDocumentsIndex).id(documentId)).value();
        } catch (Exception ex) {
            // Fallback to repository path only when direct exists check fails.
            try {
                return documentRepository.findById(documentId).isPresent();
            } catch (Exception fallbackEx) {
                log.warn("Could not verify primary document existence for id {}: {}", documentId, fallbackEx.getMessage());
                return false;
            }
        }
    }
