...
public class UserManagementController {
    ...
    @PostMapping("/users")
    public UserResponse createUser(@RequestBody @Valid UserRequest request, Principal principal) {
        return service.createUser(request, principal != null ? principal.getName() : null);
    }

    @PutMapping("/users/{id}")
    public UserResponse updateUser(@PathVariable String id, @RequestBody @Valid UserRequest request, Principal principal) {
        return service.updateUser(id, request, principal != null ? principal.getName() : null);
    }

    @DeleteMapping("/users/{id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void deleteUser(@PathVariable String id, Principal principal) {
        service.deleteUser(id, principal != null ? principal.getName() : null);
    }
}
